...
[`useLongPress`](/use-long-press), and [`useDrag`](/use-drag) are implemented.
`usePan`, `useSwipe`, `usePinch`, `useRotate`, `useHover`, and `useEdgeSwipe`
are designed and **not written**. Do not import them yet — see the
[Roadmap](/roadmap).
...
silently drops a ref that has none. See [Coexistence](/coexistence).
