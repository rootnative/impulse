...
`useTap`, `useDoubleTap`, `useLongPress`, and `useDrag` are implemented.
`usePan`, `useSwipe`, `usePinch`, `useRotate`, `useHover`, and `useEdgeSwipe`
are designed and **not written**. Do not import them yet.
...
:::info One sharp edge Impulse does not absorb yet
...
`start.value` dance is gone. `deferTo` is the relation, named for what happens
rather than for the method it calls. The `runOnJS` boundary is Impulse's job,
not yours.
...
`@rootnative/inertia-gestures` ships a `useDrag`, a `usePan`, and a `useSwipe`
too, so the names collide. The products do not. That package **requires**
`@rootnative/inertia` and hands back an `animatedStyle` that already knows
about bounds and spring-back. Impulse requires no animation library and hands
back the gesture and its values. Reach for `-gestures` to make a Motion
primitive follow a finger. Reach for Impulse for the gesture itself.
