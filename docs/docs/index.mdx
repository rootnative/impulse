...
:::warning Alpha — seven intents ship today

[`useTap`](/use-tap), [`useDoubleTap`](/use-double-tap),
[`useLongPress`](/use-long-press), [`useDrag`](/use-drag), [`usePan`](/use-pan),
[`useSwipe`](/use-swipe), and [`usePinch`](/use-pinch) are implemented.
`useRotate`, `useHover`, and `useEdgeSwipe` are designed and **not written**. Do
not import them yet — see the [Roadmap](/roadmap).
