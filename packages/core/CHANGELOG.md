...
- **`useTap` is the only intent implemented.** `useDrag`, `useSwipe`, `usePinch`, and the rest are designed but not written. Milestone 1's foundation is in place and `useTap` now exercises it; `useDrag` is the remaining Milestone 1 item, and Milestone 1's gate still needs a device pass that has not happened.
